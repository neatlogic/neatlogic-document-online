...
* 配置项模型管理详细参见[模型管理](../../3.配置管理/系统管理/模型管理.md)。
...
* 1、通过采集数据内包含关系(jsonArray数据)成员中的_OBJ_CATEGORY和_OBJ_TYPE找到目标模型（需要在对象管理中配置）。
...
注意：数据同步成功后，会将mongodb中整个document做一个hash保存起来，下一次同步时会比较hash是否发生变化，没变化的内容不会再次发起同步。即使同步后又修改了CMDB的数据，但采集数据没变化，也是不会发起同步。
...
配置模型内数据查询，详细参见[配置项查询](../配置项/配置项查询.md)
