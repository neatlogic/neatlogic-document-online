...
2. 重建模型
   ...
   先设计好[**模型**](系统管理/模型管理.md)，包括模型的继承关系、模型属性、关联关系等内容。在系统中配置完模型后，再往模型中添加[**配置项**](配置项/配置项查询.md)数据，支持[**批量导入**](配置项/批量导入.md)数据。
...
3. 重新配置资产清单和应用清单的视图(如果不使用自动化、巡检、发布、采集，请忽略跳过)
   ...
   在资产清单视图中完成映射配置，关键是“主模型”，所以在设计模型时，所有资产所属的模型应该是主模型或者主模型的子模型。详情参考[**资产清单**](../4.巡检/资源中心/资产清单.md)-资产清单数据源配置。

   应用清单<br>
   打开配置管理-资源中心-视图设置，搜索“应用清单”
   ![](images/视图管理_应用清单视图.png)
   应用清单视图配置请参考[**应用清单**](../4.巡检/资源中心/应用清单.md)-应用清单数据源配置。
...
普通模型才能添加配置项，抽象模型只能用于继承，关于各类型模型的概念，请参考[**模型管理**](系统管理/模型管理.md)。
...
## 6、资产清单无法正常加载
配置管理 -> 资源中心 -> 点击查看视图，看映射配置是否完整，默认出厂是有对应的配置，倘若配置模型存在模型删除、字段删除会导致[视图](../4.巡检/资源中心/视图设置.md)需要重新配置和映射。
...
## 10、配置管理是否提供接口
系统管理 -> API管理 -> 配置管理 ，所有API接口都存在，按需获取。
...
## 13、资产清单如何根据环境数据过滤
资产清单是否能够根据环境过滤数据？怎么过滤？下面将从四个相关的点来讲解。
...
4. 检查用户是否有模型权限
   ...
   在模型层面可以做数据过滤，用户有模型的任意一项权限，就可以查看到该模型下所有的配置项，详情参考 [模型管理](../3.配置管理/系统管理/模型管理.md) 的授权功能。

5. 检查团体授权
   
   在模型内过滤配置项，通过团体管理来实现，做了过滤的团体详情参考[团体管理](../3.配置管理/系统管理/团体管理.md)。<br>

总结：1 的配置启用后，才能做资产清单数据过滤配置。资产清单数据是取 3、4、5 这三个维度权限过滤后的并集。完成 2 中角色的配置，并且保证角色的规则生效，即可完成根据环境过滤资产清单数据。
